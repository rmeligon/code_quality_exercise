...
class TradeExecutionService
  ...
  def execute_order(side, size, currency, counter_currency, date, price, order_id)
    validate_order_parameters(side, size, currency, counter_currency, date, price, order_id)
    ...
    File.open('D:\data\CV Rafa\QA test Kantox/errors.log', 'a') do |f|
      f.puts "Execution of #{order_id} failed." + msg.to_s
    end
  end
  ...
  def define_lp(size)
    ...
    elsif (size >= 10_000 && size < 100_000)
  ...
  def validate_order_parameters(side, size, currency, counter_currency, date, price, order_id)
    validates :side, inclusion: { in: ['sell', 'buy']}
    validates :size, numericality: { greater_than: 0 }
    validates :currency, length: { is: 3}
    validates :counter_currency, length: { is: 3}
  ...
  def issue_rest_market_trade(side, size, currency, counter_currency, date, price, order_id)
    ...
    #puts "respuesta "  + response.code.to_s
